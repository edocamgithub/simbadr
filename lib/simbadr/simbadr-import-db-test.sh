#!/bin/bash
##################################################################
#  File: simbadr-import-db.sh 	        Realease 202210182231pm
#  Version: 0.0.2					 
#
...
#                 ---------------------------
#
#  Written by Eduardo M. Araujo. - versão 0.0.2
...
select_img_system_OS () {

search_osname="Station"
osname_in=$os_name

echo "$search__osname" > /tmp/xxx.txt
echo $osname_in > /tmp/yyy.txt


case "$search_osname" in

 Station )  
 if $osname_in = "Windows"
	then
 			setup_img_OS=$($baseDIR_LIB_/getimage.sh --hostWorkstation 2 | cut -d ":" -f"1" | cut -d "/" -f "7-10" | sed 's/simbadr/../')          
fi

 if $osname_in = "Ubuntu"
	then
 			setup_img_OS=$($baseDIR_LIB_/getimage.sh --hostWorkstation 3 | cut -d ":" -f"1" | cut -d "/" -f "7-10" | sed 's/simbadr/../')          
fi
...
if $osname_in = "Windows"
	then
 			setup_img_OS=$($baseDIR_LIB_/getimage.sh --hostWindows 0 | cut -d ":" -f"1" | cut -d "/" -f "7-10" | sed 's/simbadr/../')          
fi

if $osname_in = "Ubuntu"
	then
 			setup_img_OS=$($baseDIR_LIB_/getimage.sh --hostLinux 0 | cut -d ":" -f"1" | cut -d "/" -f "7-10" | sed 's/simbadr/../')          
fi

if $osname_in = "FreeBSD"
	then
 			setup_img_OS=$($baseDIR_LIB_/getimage.sh --hostBsd 1 | cut -d ":" -f"1" | cut -d "/" -f "7-10" | sed 's/simbadr/../')          
fi
...
if $osname_in = "Windows"
	then
 			setup_img_OS=$($baseDIR_LIB_/getimage.sh --hostLaptop 2 | cut -d ":" -f"1" | cut -d "/" -f "7-10" | sed 's/simbadr/../')          
fi

if $osname_in = "BSD"
# BSD
	then
 			setup_img_OS=$($baseDIR_LIB_/getimage.sh --hostLaptop 3 | cut -d ":" -f"1" | cut -d "/" -f "7-10" | sed 's/simbadr/../')          
fi

if $osname_in = "MacX"
	then
# MacX
 			setup_img_OS=$($baseDIR_LIB_/getimage.sh --hostLaptop 4 | cut -d ":" -f"1" | cut -d "/" -f "7-10" | sed 's/simbadr/../')          
fi

if $osname_in = "Ubuntu"
	then
# Ubuntu
 			setup_img_OS=$($baseDIR_LIB_/getimage.sh --hostLaptop 5 | cut -d ":" -f"1" | cut -d "/" -f "7-10" | sed 's/simbadr/../')          
fi

if $osname_in = "MacOS"
	then
# MacClassic
 			setup_img_OS=$($baseDIR_LIB_/getimage.sh --hostLaptop 6 | cut -d ":" -f"1" | cut -d "/" -f "7-10" | sed 's/simbadr/../')          
fi
           ;;
...
export_XML_header () {
echo "<group>" > /tmp/simbadrdb.header
}

export_XML_footer () {
echo "</group>" > /tmp/simbadrdb.footer
}

export_XML_DataBase () { 

echo "   <host id="'"'$ipaddress_'"'">" >> /tmp/simbadrdb.tmp
echo "   <hostname>$hostname_</hostname>
   ...
   echo "<setupimg img_os="'"'$setup_img_os'"' "img_device="'"'$setup_img_device'"' "img_status="'"'$setupimg_on_'"' "/>"
  echo " </host>" 
